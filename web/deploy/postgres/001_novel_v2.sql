...
CREATE TABLE IF NOT EXISTS context_manifests (id TEXT PRIMARY KEY, project_id TEXT NOT NULL REFERENCES novel_projects(id) ON DELETE CASCADE, retrieval_run_id TEXT REFERENCES retrieval_runs(id), source_revision_ids TEXT[] NOT NULL DEFAULT '{}', token_budget INTEGER NOT NULL, truncation_reason TEXT, fingerprint TEXT NOT NULL, payload JSONB NOT NULL DEFAULT '{}', created_at TIMESTAMPTZ NOT NULL DEFAULT now());
ALTER TABLE context_manifests ADD COLUMN IF NOT EXISTS payload JSONB NOT NULL DEFAULT '{}';
CREATE TABLE IF NOT EXISTS learning_assessments (id TEXT PRIMARY KEY, project_id TEXT NOT NULL REFERENCES novel_projects(id) ON DELETE CASCADE, source JSONB NOT NULL, conclusion TEXT NOT NULL, payload JSONB NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now());
