...
export function buildCharacterEnrichmentPrompt(input: CharacterEnrichmentPromptInput): string {
  return [
    ...
    `- motivationDelta：本章动机变化（1-2 句话，如"从复仇转为保护妹妹，意识到仇恨会吞噬自己"）。若无变化填"本章无明显动机变化"。`,
    `- newKnowledge：本章中该角色新获得的知识（谁知道了什么、何时知道）。每条包含 description（知识描述）+ evidence（正文逐字证据）。若无新知识返回空数组。`,
    ...
    `- motivationDelta 若本章该角色动机无变化，填"本章无明显动机变化"，不要强行编造。`,
    "",
