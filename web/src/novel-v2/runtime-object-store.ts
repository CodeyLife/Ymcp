...
export async function bindRuntimeObjectStore(
  ...
  if (missing.length) {
    const samples = missing.slice(0, 5).map((item) => `${item.reference}(${item.objectKey})`).join("、");
    throw new Error(`对象存储完整性检查失败：${missing.length} 个活跃引用对象不可读：${samples}`);
  }
  await repository.assertRuntimeObjectStoreIdentity(identity);

  console.info(`[object-store] ${service} 已绑定 ${identity.backend} ${identity.location}，已校验 ${references.length} 个活跃引用对象`);
}
