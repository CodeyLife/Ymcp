...
describe("V2 repository management APIs", () => {
  ...
  it("loads focused plan regeneration context from current approved projections", async () => {
    const { repository, calls } = createRepository([
      ...
      { rows: [{ task_key: "characters", payload: { title: "人物", summary: "作者修订后的当前人物设定" } }], rowCount: 1 },
    ]);
    await expect(repository.getFoundationWorkContext("p1", [], "relations")).resolves.toEqual({
      project: { title: "测试项目", metadata: { genre: "悬疑" } },
      priorArtifacts: [{ taskKey: "characters", title: "人物", summary: "作者修订后的当前人物设定" }],
    });
    expect(calls[1].sql).toContain("FROM project_plan_sections ps");
    expect(calls[1].sql).toContain("ps.status='approved'");
    expect(calls[1].params).toEqual(["p1", ["characters", "worldview"]]);
  });
