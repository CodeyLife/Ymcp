...
import { assessRuntimeLearningWithModel, parseRuntimeLearningAssessmentV2 } from "../../../novel-v2/learning-assessment";
import type { Artifact, Review } from "../../../novel-v2/protocol";
import type { ModelGateway, ModelUsage } from "../../../novel-v2/model-gateway";
...
describe("V2 runtime learning assessment", () => {
  ...
  it("uses model output only after parser validation succeeds", async () => {
    const model: ModelGateway = {
      ...
      embed: async () => ({ vectors: [], usage }),
      rerank: async () => ({ scores: [], usage }),
    };
  ...
  it("falls back to no-shared-learning when model output fails validation", async () => {
    const model: ModelGateway = {
      generateStructured: async () => ({ value: { conclusion: "propose-improvement", symptom: "视角越界" }, usage }),
      generateText: async () => ({ text: "", usage }),
      embed: async () => ({ vectors: [], usage }),
      rerank: async () => ({ scores: [], usage }),
    };
