...
export default function CraftRuleGovernance({ projectId }: { projectId: string }) {
  ...
  async function changeStatus(action: "promote" | "rollback") {
    ...
    setBusy(action);
    try {
      if (action === "promote") await promoteCraftRuleCandidate(active.id);
      else await rollbackCraftRuleCandidate(active.id);
      message.success(action === "promote" ? "新版本已生效" : "已切回评测前版本");
