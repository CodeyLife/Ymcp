...
import { CheckCircleOutlined, ClockCircleOutlined, RightOutlined, RobotOutlined } from "@ant-design/icons";
...
export default function AIWorkbench({ projectId, document, targetLabel, collapsed, onToggle }: { projectId: string; document?: ManuscriptDocument; scope: NovelAssistantScope; targetLabel?: string; collapsed: boolean; onToggle: () => void }) {
  ...
  return <aside className="novel-ai-panel novel-task-history">
    ...
    <section className="novel-ai-history"><header><div><strong>候选历史</strong><small>等待确认的结构化建议</small></div><Badge count={pending} showZero /></header>{proposals.slice(0, 8).map((proposal) => <article key={proposal.id}><div><Tag color={proposal.status === "pending" ? "gold" : proposal.status === "accepted" ? "green" : proposal.status === "partially_accepted" ? "blue" : undefined}>{PROPOSAL_STATUS_LABEL[proposal.status] ?? proposal.status}</Tag><time>{new Date(proposal.createdAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</time></div><strong>{proposal.title}</strong><p>{proposal.items.length ? `${proposal.items.length} 个结构化候选项` : proposal.previewMarkdown.slice(0, 100)}</p></article>)}{!proposals.length && <p className="novel-ai-empty-copy">尚无候选记录</p>}</section>
    <section className="novel-ai-history"><header><div><strong>模型任务</strong><small>最近执行记录</small></div></header>{agents.slice(0, 8).map((agent) => <article key={agent.id}><div><span className={`novel-agent-status ${agent.status}`}>{agent.status === "completed" ? <CheckCircleOutlined /> : <ClockCircleOutlined />}{AGENT_STATUS_LABEL[agent.status] ?? agent.status}</span><time>{new Date(agent.createdAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</time></div><strong>{agent.goal}</strong><p>{agent.usage ? `${(agent.usage.inputTokens + agent.usage.outputTokens).toLocaleString()} tokens` : "等待用量统计"}</p></article>)}</section>
  </aside>;
}
