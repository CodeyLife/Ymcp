...
${formatContextPacket(packet)}`,
    ...
    const artifact = await ctx.saveArtifact(run, {
      ...
      kind: "fact-delta",
      title: "事实与状态差异",
      contentMarkdown: `# 事实差异\n\n${data.summary}\n\n${facts.map((item) => `- ${item.targetTable}.${item.field}：${formatFactCandidateValue(item)}\n  - 证据：${item.evidence}\n  - 置信度：${Math.round(item.confidence * 100)}%${item.conflict ? " · 存在冲突" : ""}`).join("\n") || "未提取到变化"}${Object.values(prepared).slice(1).some((count) => typeof count === "number" && count > 0) ? `\n\n# 提取清理\n\n已过滤：重复人物 ${prepared.discardedDuplicateCharacterCount} 项、元叙事空事实 ${prepared.discardedMetaAbsenceCount} 项、无法投影 ${prepared.discardedUnprojectableCount} 项、批内重复 ${prepared.discardedDuplicateFactCount} 项、无效 record ${prepared.discardedInvalidRecordCount} 项、无效目标（含幻觉 targetId） ${prepared.discardedInvalidUpdateTargetCount} 项、重复关系 ${prepared.discardedDuplicateRelationCount} 项、非法 targetTable ${prepared.discardedInvalidTargetTableCount} 项、关系缺 endpoints ${prepared.discardedRelationMissingEndpointsCount} 项、人物缺 name ${prepared.discardedCharacterMissingNameCount} 项、update 缺 targetId ${prepared.discardedUpdateMissingTargetIdCount} 项。` : ""}`,
      structuredData: {
        ...
        discardedInvalidRecordCount: prepared.discardedInvalidRecordCount,
        discardedInvalidUpdateTargetCount: prepared.discardedInvalidUpdateTargetCount,
        discardedDuplicateRelationCount: prepared.discardedDuplicateRelationCount,
        discardedInvalidTargetTableCount: prepared.discardedInvalidTargetTableCount,
        discardedRelationMissingEndpointsCount: prepared.discardedRelationMissingEndpointsCount,
        discardedCharacterMissingNameCount: prepared.discardedCharacterMissingNameCount,
        discardedUpdateMissingTargetIdCount: prepared.discardedUpdateMissingTargetIdCount,
      },
      model: project.settings.textModel,
      skillRefs: skills.skills.map((item) => `${item.skillId}@${item.version}`),
