  ...
  return <div className={`novel-architecture-data-editor${preview ? " preview" : ""}${readOnly ? " read-only" : ""}`}>
    <div className="novel-architecture-form">
      <section className="novel-architecture-meta">
        <label><span>结构方法</span><Select value={value.framework} options={FRAMEWORK_OPTIONS} disabled={readOnly} onChange={(framework) => update({ framework })} /></label>
        <label><span>架构状态</span><Select value={value.status} options={STATUS_OPTIONS} disabled={readOnly} onChange={(status) => update({ status })} /></label>
      </section>
      <label className="novel-architecture-core-field"><span>核心问题</span><Input.TextArea readOnly={readOnly} rows={3} placeholder="贯穿全书、最终必须回答的问题" value={value.centralQuestion} onChange={(event) => update({ centralQuestion: event.target.value })} /></label>
      <label className="novel-architecture-core-field"><span>核心冲突</span><Input.TextArea readOnly={readOnly} rows={3} placeholder="推动主角持续行动的主要矛盾" value={value.centralConflict} onChange={(event) => update({ centralConflict: event.target.value })} /></label>
      <label className="wide"><span>全书梗概</span><Input.TextArea readOnly={readOnly} rows={6} placeholder="用一段连续叙述概括起因、升级、转折与结局" value={value.synopsis} onChange={(event) => update({ synopsis: event.target.value })} /></label>
    </div>
