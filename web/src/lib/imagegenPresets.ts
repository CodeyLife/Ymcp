...
export const IMG2IMG_REFERENCE_GUIDES: Img2ImgReferenceGuide[] = [
  {
    ...
    label: "宽松人物参考",
    fragment:
      `请把参考图当作 casting reference（选角参考 / 人物灵感板），不是 source image edit（原图编辑）、不是换脸任务，也不是同一人物的高保真复刻。

核心目标：生成一个受参考图启发的新人物画面。结果应让人联想到参考图的人物类型和气质方向，但不需要、也不应该像同一张脸的精修版或相邻照片。

只参考这些抽象低精度信息：
- 大致年龄段、肤色范围、脸型家族、五官比例倾向、整体气质。
- 可参考发色或发量带来的气质印象，但头发必须重新设计。

必须重新设计这些内容：
- 新的头发轮廓、刘海或分缝、发束走向、鬓角和局部发丝形态。
- 新的表情结构、视线方向、头部角度、脸部细节和局部光影。
- 新的服装、配饰、身体姿势、手部动作、场景、构图、镜头距离、拍摄角度和光线。

生成策略：先根据参考图抽象出“人物类型 + 气质方向”，再围绕新画面描述重新创造角色照片。以新画面描述为最高优先级；如果参考图细节与新描述冲突，忽略参考图细节。

最终画面应像一次全新拍摄或全新角色写真，而不是原图 PS、局部编辑、重绘、换脸、同一人证件级复刻、相邻帧或相机连拍。允许脸部细节、发型细节和表情结构明显变化，只要人物类型和整体气质仍有参考图的影子即可。

新画面描述：`,
    description: "宽松选角参考：只借人物类型和气质，重新设计脸部细节、头发、服装、场景和构图",
  },
  {
