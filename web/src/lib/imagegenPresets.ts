...
export const IMG2IMG_REFERENCE_GUIDES: Img2ImgReferenceGuide[] = [
  {
    ...
    label: "人物一致性",
    fragment:
      `请把参考图作为“同一人物身份参考”，目标是生成同一个可识别人物的一张新照片，而不是原图编辑、局部重绘、相邻帧或同角度复刻。

核心目标：保持人物识别度，同时改变拍摄状态。人物应仍能被认出是参考图中的同一个人，但服装、姿势、动作、头部朝向、视线、表情、机位、构图、场景和光线都应按新画面重新生成。
...
最终画面应像同一人物、同一发型大类在新地点、新动作、新服饰、新机位下拍摄的新照片：身份稳定，发型自然连续，但拍摄状态明显不同。
...
新画面描述是最高优先级。新画面描述：`,
  ...
  },
  {
    ...
    label: "同场景二次拍摄",
    fragment:
      `请把参考图作为“三要素连续性参考”，而不是待编辑原图。目标是生成同一个人物、同一套服装、同一个地点附近里的另一张独立照片。
...
画面要求：
- 看起来像同一人物在同一个大致环境下拍摄的一张全新照片，而不是原图的 PS、局部编辑、换脸、重绘、轻微变体、相邻帧或相机连拍。
- 不要复刻参考图的原始发丝、刘海、姿势、表情、眼神、手势、站位、构图、镜头角度或裁切。
- 在人物、服装、大致场景一致三要素稳定的前提下，优先增加姿势、表情、动作、发丝自然差异和机位变化。
