...
export const IMG2IMG_REFERENCE_GUIDES: Img2ImgReferenceGuide[] = [
  {
    ...
    label: "人物一致性变体",
    fragment:
      `请把参考图作为“同一人物身份参考”，目标是生成同一个可识别人物的一张新照片，而不是原图编辑、局部改衣、重绘、换脸、相邻帧或同角度自拍复刻。

核心目标：保持人物识别度，同时改变拍摄状态。人物应仍能被认出是参考图中的同一个人，但姿势、动作、头部朝向、视线、表情、机位、构图、场景和光线都应按新画面重新生成。

用户的新画面描述是最高优先级。如果新画面描述包含地点、服装、动作或场景词，必须采用这些新设定，并丢弃参考图里的原始地点、服装、构图、光线和拍摄角度。例如用户写“来福士，黑色吊带”时，应生成来福士/商场/城市综合体环境中的黑色吊带人像，不要保留参考图的卧室、床、白墙或室内自拍背景。

需要保持的人物识别信息：
- 年龄感、肤色范围、脸型轮廓、五官比例、眉眼鼻唇的大体结构和整体气质。
- 发型大类和整体发型结构，例如发长、发量感、是否有刘海、是否扎起、披落/蓬松/凌乱/柔顺的整体气质。

头发自然变化规则：
- 保持同一个发型大类，但不要逐根复制参考图的发丝位置。
- 遮脸发丝、刘海缝隙、碎发位置、鬓角形态和发尾走向应随新的姿势、动作、风向、重力、头部朝向和镜头角度自然变化。
- 不要把遮在脸上的具体发丝当成面部身份特征；身份来自脸型和五官比例，不来自某几根固定发丝。

必须强制变化的拍摄状态：
- 人脸在画面中的位置、大小、朝向和头部角度必须不同于参考图，不能沿用同一脸部位置或同一近景自拍裁切。
- 重新选择身体姿势、肩颈方向、手部动作、视线方向和表情结构。
- 重新选择镜头距离、拍摄角度、构图重心、背景空间和光线。
- 如果用户没有指定姿势或机位，请主动选择一个明显不同于参考图的自然姿势和机位。
