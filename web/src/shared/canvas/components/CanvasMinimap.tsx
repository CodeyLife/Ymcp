...
type MinimapNode = Pick<CanvasNode, "position" | "width" | "height" | "kind" | "id">;

/**
 * 画布小地图：根据所有节点计算世界包围盒，按比例缩放渲染节点矩形 + 当前视口框。
 * 点击/拖拽小地图可平移视口到对应世界坐标。
 *
 * `nodeColor` 允许业务侧按 kind 指定小地图节点颜色，默认统一使用主题 muted 色。
 */
export function CanvasMinimap({
  ...
  nodeColor?: (node: MinimapNode) => string;
}) {
    ...
    >
      ...
      >
        {nodes.map((node) => {
          ...
          const color = nodeColor?.(node) ?? theme.node.muted;
          return (
            <div
              key={node.id}
