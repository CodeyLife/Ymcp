...
import { Alert, Button, Input, Modal, Popconfirm, Space, Spin, Tag, message } from "antd";
...
type ArcPlan = { title: string; objective: string; entryState: string; centralConflict: string; development: string[]; resolution: string; exitState: string; plotThreadRefs: string[]; foreshadowingRefs: string[]; expectedChapterCount: number; authorIntent?: string };
type StoryArc = { id: string; ordinal: number; planningStatus: "generating" | "awaiting-review" | "approved" | "stale" | "failed"; executionStatus: "planned" | "active" | "completed" | "abandoned"; arc: ArcPlan; chapters: Chapter[]; sourceArtifactId?: string; blueprintArtifactId?: string; contextFingerprint?: string; editRevision: number; updatedAt: string };
...
export default function StoryArcPanel({ projectId, onApplied }: { projectId: string; onApplied?: () => void }) {
  ...
  async function startNext() {
    await action("next", async () => {
      await readJson(`/v2/projects/${encodeURIComponent(projectId)}/story-arcs/next`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ authorIntent: authorIntent.trim() || undefined }) });
  ...
  async function saveEdit() {
    ...
    await action("edit", async () => {
      ...
      await readJson(`/v2/projects/${encodeURIComponent(projectId)}/story-arcs/${encodeURIComponent(editArc.id)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ bundle: { arc: { ...editArc.arc, expectedChapterCount: chapters.length }, chapters } }) });
      setEditArc(undefined); message.success("整弧蓝图已保存，需重新审核");
    });
  }
  ...
  async function rebase() {
    ...
    await action("rebase", async () => {
      await readJson(`/v2/projects/${encodeURIComponent(projectId)}/story-arcs/${encodeURIComponent(selected.id)}/rebase`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ authorIntent: authorIntent.trim() || selected.arc.authorIntent }) });
  ...
  function addChapter() {
    setEditArc((current) => current ? { ...current, chapters: [...current.chapters, { index: current.chapters.length + 1, title: "新章节", summary: "", chapterPurpose: "", dramaticQuestion: "", emotionalMovement: "", stateDeltaBudget: "", optionalBeats: [], scenes: [], continuityConstraints: [], setupRefs: [], payoffRefs: [], closingForce: "", freedom: "允许在章节功能和连续性约束内自由组织场景。" }] } : current);
  }

  if (loading) return <div className="novel-plan-loading"><Spin /></div>;
  if (!arcs.length) return <motion.section className="novel-plan-empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}><div className="novel-plan-empty-mark"><FileAddOutlined /></div><h2>规划第一个故事弧</h2><p>宏观规划确定长期方向后，每次只展开一个连续的小故事及其全部章节蓝图。</p><Button type="primary" size="large" icon={<PlusOutlined />} onClick={() => setIntentOpen(true)}>生成故事弧</Button><Modal title="本弧创作意图" open={intentOpen} onCancel={() => setIntentOpen(false)} onOk={() => void startNext()} confirmLoading={busy === "next"} okText="开始生成"><Input.TextArea value={authorIntent} onChange={(event) => setAuthorIntent(event.target.value)} autoSize={{ minRows: 4, maxRows: 8 }} placeholder="可选：本弧想重点探索的冲突、关系或氛围" /></Modal></motion.section>;
  ...
  return <motion.section className="novel-arc-workspace" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <main className="novel-arc-reader">
      <header className="novel-plan-reader-head"><div><span className="novel-eyebrow">故事弧 {selected.ordinal}</span><h2>{selected.arc.title}</h2></div><Space wrap><Tag>{PLAN_LABEL[selected.planningStatus]}</Tag><Tag>{EXEC_LABEL[selected.executionStatus]}</Tag></Space></header>
    ...
    <aside className="novel-plan-actions"><div><span className="novel-section-label">整弧状态</span><strong>{PLAN_LABEL[selected.planningStatus]}</strong><small>{selected.chapters.length} 章 · 修订 {selected.editRevision}</small></div><Space direction="vertical" style={{ width: "100%" }}>
      {selected.planningStatus !== "generating" && !["completed", "abandoned"].includes(selected.executionStatus) && <Button block icon={<EditOutlined />} onClick={() => setEditArc(clone(selected))}>编辑整弧蓝图</Button>}
    ...
    <Modal title="本弧创作意图" open={intentOpen} onCancel={() => setIntentOpen(false)} onOk={() => void startNext()} confirmLoading={busy === "next"} okText="开始生成"><Input.TextArea value={authorIntent} onChange={(event) => setAuthorIntent(event.target.value)} autoSize={{ minRows: 4, maxRows: 8 }} /></Modal>
