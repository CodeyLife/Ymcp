import type { LearningAssessment, LearningImprovementProposal } from "../features/novel/types";
...
export type RuntimeLearningAssessment = LearningAssessment;
export type RuntimeLearningImprovementProposal = LearningImprovementProposal;
...
export function parseRuntimeLearningAssessment(value: unknown, options: { requireTargetBaseline?: boolean } = {}): RuntimeLearningAssessment {
  ...
  const targetContentFingerprint = typeof proposal.targetContentFingerprint === "string" && /^[a-f0-9]{64}$/i.test(proposal.targetContentFingerprint)
    ? proposal.targetContentFingerprint.toLowerCase()
    : undefined;
  if (options.requireTargetBaseline && (!targetVersion || !targetContentFingerprint)) {
    throw new Error("learning.proposal 必须携带审核时 targetVersion 与 targetContentFingerprint");
  }
  if ((targetVersion && !targetContentFingerprint) || (!targetVersion && targetContentFingerprint)) {
    throw new Error("learning.proposal 的 targetVersion 与 targetContentFingerprint 必须同时提供");
  }
  return {
    ...
    proposal: {
      targetKind,
      targetId: requiredText(proposal.targetId, "proposal.targetId"),
      targetVersion,
      targetContentFingerprint,
      afterText,
      rationale: requiredText(proposal.rationale, "proposal.rationale"),
      observedSymptom: requiredText(proposal.observedSymptom, "proposal.observedSymptom"),
      failingLayer: requiredText(proposal.failingLayer, "proposal.failingLayer"),
      intendedBenefits: requiredTextList(proposal.intendedBenefits, "intendedBenefits"),
      boundaries: requiredTextList(proposal.boundaries, "boundaries"),
      nonGoals: requiredTextList(proposal.nonGoals, "nonGoals"),
      regressionRisks: requiredTextList(proposal.regressionRisks, "regressionRisks"),
    },
  };
}
