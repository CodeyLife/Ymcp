...
def test_complete_copy_helpers_keep_common_contract_suffixes():
    ...
    assert blocked.endswith('若宿主不支持 Elicitation，则本 tool 应返回 blocked，并要求宿主以 handoff.options 渲染真实可交互菜单；普通文本列表或 assistant 代渲染不能替代交互选择。')
...
def test_complete_tool_descriptions_share_blocked_contract():
    ...
    for description in descriptions.values():
        assert description.endswith('若宿主不支持 Elicitation，则本 tool 应返回 blocked，并要求宿主以 handoff.options 渲染真实可交互菜单；普通文本列表或 assistant 代渲染不能替代交互选择。')
