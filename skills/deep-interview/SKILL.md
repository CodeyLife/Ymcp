...
- `clarified_artifact` 只在 `ydeep_complete` 阶段产出
- 下一步动作必须以 `handoff.options` 为唯一权威来源

换句话说：

- **skill 负责澄清方法**
- **tool / host 负责阶段 gate、complete 阶段与 handoff 菜单**
...
### Phase 5：进入 `ydeep_complete`
当你认为本轮澄清已经足够进入下一阶段时，调用 `ydeep_complete`。

请记住：

- `ydeep_complete` 返回的是**收口后的 handoff 菜单**
- 当前合法下一步只有：
  - `yplan`
  - `refine_further`
...
complete 阶段以后，下一步动作必须以 `handoff.options` 为唯一权威来源。
...
在调用 `ydeep_complete` 前，你的总结可优先覆盖这些内容：
