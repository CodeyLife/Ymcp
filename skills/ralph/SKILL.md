...
> Boundary note: This skill describes how an upper-layer agent/host can consume Ymcp workflow outputs. It does **not** mean the Ymcp MCP server will directly execute all actions below；Ymcp 的 `ralph` tool 本身只负责基于证据做继续/修复/完成判断。
